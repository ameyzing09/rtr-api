...
// Check if user can manage tracking (ADMIN, HR)
// These roles can move stages, update status, attach applications
export function canManageTracking(role: string): boolean {
  return ['SUPERADMIN', 'ADMIN', 'HR'].includes(role);
}

// Check if user can view tracking (ADMIN, HR, INTERVIEWER)
// These roles can view state, history, and board
export function canViewTracking(role: string): boolean {
