...
// Check if user can manage evaluations (ADMIN, HR)
// These roles can create/complete evaluations, add participants
export function canManageEvaluations(role: string): boolean {
  return ['SUPERADMIN', 'ADMIN', 'HR'].includes(role);
}

// Check if user can view evaluations (ADMIN, HR, INTERVIEWER)
// These roles can view evaluation instances, responses, signals
export function canViewEvaluations(role: string): boolean {
  return ['SUPERADMIN', 'ADMIN', 'HR', 'INTERVIEWER'].includes(role);
}

// Check if user can manage settings (SUPERADMIN, ADMIN)
// These roles can manage templates and configurations
