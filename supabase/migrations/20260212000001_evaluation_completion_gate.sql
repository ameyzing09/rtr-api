...
-- ============================================================================
-- Replaces the legacy feedback gate (stage_feedback + requires_feedback) in
-- execute_action_v2 with an evaluation-based gate driven by stage_evaluations
-- and evaluation_instances.
--
-- Decision: Fail-open — if a stage has no required stage_evaluations,
-- actions are NOT blocked.
--
-- Part 1: Partial index for gate query performance
-- Part 2: Replace execute_action_v2 Step 7 (feedback → evaluations)
-- Part 3: Deprecation comment on requires_feedback
-- ============================================================================
...
CREATE INDEX IF NOT EXISTS idx_stage_evaluations_required_gate
  ON stage_evaluations(stage_id, tenant_id)
  WHERE required = true AND is_active = true;

-- ============================================================================
-- PART 2: Replace execute_action_v2 — Step 7 swapped to evaluation gate
-- ============================================================================
...
BEGIN
  ...
  WHERE se.stage_id = v_state.current_stage_id
    ...
    AND ei.id IS NULL;  -- no completed instance
...
COMMENT ON FUNCTION execute_action_v2 IS
  'Execute an action on an application with evaluation completion gate and signal condition gates.
   Step 7 now checks stage_evaluations + evaluation_instances (fail-open).
   Legacy feedback gate (stage_feedback + requires_feedback) removed.';

-- ============================================================================
-- PART 3: Deprecation comment on requires_feedback
-- ============================================================================

COMMENT ON COLUMN tenant_stage_actions.requires_feedback IS
  'DEPRECATED: Gating now driven by stage_evaluations.required + evaluation_instances completion. Ignored by execute_action_v2.';
