# Lambda function for Auth app
# Handles: /auth/login, /auth/federate, /auth/refresh, /auth/logout
...
resource "aws_lambda_function" "auth" {
  ...
  environment {
    variables = {
      ...
      JWT_ISSUER     = "rtr-api"
      JWT_AUDIENCE   = "rtr-app"
      JWT_EXPIRES_IN = var.env == "prod" ? "15m" : "1h"
...
resource "aws_cloudwatch_metric_alarm" "auth_errors" {
  ...
  statistic           = "Sum"
  threshold           = 10
  alarm_description   = "Auth Lambda function error rate"
...
resource "aws_cloudwatch_metric_alarm" "auth_duration" {
  count = var.env == "prod" ? 1 : 0
  ...
  alarm_description   = "Auth Lambda function duration"
  treat_missing_data  = "notBreaching"
